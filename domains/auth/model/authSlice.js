...
const authSlice = createSlice({
  ...
  reducers: {
    setCredentials: (state, action) => {
      const { user, token, notificationData,branch,general_setting,translation,language_code } = action.payload;
      state.user = user;
      state.token = token;
      state.isAuthenticated = true;
      state.notificationData = notificationData;
      state.branch = branch;
      state.general_setting = general_setting;
